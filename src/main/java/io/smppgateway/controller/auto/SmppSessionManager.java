 ...
 * When SMPP Server wants to send out a MO message, it requests a session by calling getNextServerSession.
 * A session is chosen based on round robin algorithm.
 *
...
public class SmppSessionManager {
    ...
    public synchronized void removeServerSession(SmppServerSession session) {
        ...
        if (!removed) {
            logger.warn("Failed to remove session {}, Session not found.", session);
        }
    }
